...
from .client import NvdClient, to_summary, to_detail
from .models import CVE_ID_PATTERN, RiskReport, SeverityCounts, Severity, TopCve
...
def _recommended_action(counts: SeverityCounts) -> str:
    ...
    if counts.medium > 0:
        return "MEDIUM PRIORITY: Schedule remediation within the next maintenance window."
    if counts.low > 0:
...
async def lookup_cve(cve_id: str) -> dict[str, Any]:
    ...
    Returns:
        ...
        Returns an error dict if the CVE is not found.
    """
...
async def search_cves(keyword: str, max_results: int = 10) -> list[dict[str, Any]]:
    ...
    Args:
        keyword: Product name or search term, e.g. "Apache Log4j" or "OpenSSL".
